...
namespace N_Glacier {
	...
	MassBalance* MassBalanceFactory::make(const PhysicsCoreParams& aPhysCoreParams, const N_Configuration::Component& aMassBalance)
	{ 
	...
	MassBalance* MassBalanceFactory::make(const PhysicsCoreParams& aPhysCoreParams)
	{
		LOG_WRN("No configuration provided. Setting MassBalance to zero.");
		return new ZeroMB(aPhysCoreParams);
	}
